...
AMiyamotoIoriController::AMiyamotoIoriController()
{
	...
	static ConstructorHelpers::FObjectFinder<UInputAction> ServantChangeActionFinder(
		TEXT("/Script/EnhancedInput.InputAction'/Game/Blueprint/PlayableCharacter/Input/IA_ServantChange.IA_ServantChange'"));
...
void AMiyamotoIoriController::BeginPlay()
{
	...
	if (MiyamotoIori && DummyCharacterClass)
	{
		...
		FActorSpawnParameters SpawnParams;
		SpawnParams.SpawnCollisionHandlingOverride = ESpawnActorCollisionHandlingMethod::AdjustIfPossibleButAlwaysSpawn;
		DummyCharacter = GetWorld()->SpawnActor<AServantBaseCharacter>(DummyCharacterClass, NewSpawnLocation, SpawnRotation, SpawnParams);
		if (DummyCharacter)
		{
...
void AMiyamotoIoriController::Tick(float DeltaTime)
{
	...
	if (bIsPendingAutoSwap && CurPlayableCharacter != MiyamotoIori)
	{
		AServantBaseCharacter* CurrentServant = Cast<AServantBaseCharacter>(CurPlayableCharacter);
		//서번트가 교체 가능한 상태인지 확인
		if (CurPlayableCharacter && CurrentServant->CanForceSwap())
		{
			bIsPendingAutoSwap = false;
			SwapWithServant(GetFocusedServantCharacter());
		}
	}
}
...
void AMiyamotoIoriController::SetupResonanceSystem(UResonanceComponent* ReadyResonanceComp)
{
	if (!ReadyResonanceComp)
		return;
	if (SaberCharacter)
	{
		FName SaberName = SaberCharacter->GetServantName();
...
AServantBaseCharacter* AMiyamotoIoriController::GetFocusedServantCharacter()
{
	AFatePlayerState* PS = GetPlayerState<AFatePlayerState>();
	if (PS && PS->ResonanceComponent)
	{
		FName FocusedName = PS->ResonanceComponent->GetFocusedServantName();
		if (PartyServantMap.Contains(FocusedName))
		{
			return PartyServantMap[FocusedName];
		}
	}
	return nullptr;
}

AServantBaseCharacter* AMiyamotoIoriController::GetPartyServantCharacterByName(FName Name)
{
	return PartyServantMap[Name];
}
...
{
	if (isUIMode || UIMode != UIMODE::NONE)
		return;
	if(CurPlayableCharacter == MiyamotoIori)
	{
		UGameplayStatics::SetGlobalTimeDilation(GetWorld(), 0.0f);
		isUIMode = true;
		UIMode = UIMODE::SWORDSTANCE;
...
void AMiyamotoIoriController::ChangeStanceCompletedInput(const FInputActionValue& value)
{
	...
	if (isUIMode == false || UIMode != UIMODE::SWORDSTANCE)
		return;
	if (CurPlayableCharacter == MiyamotoIori)
	{
...
void AMiyamotoIoriController::SkillSelectInput(const FInputActionValue& value)
{
	...
	if(MoveValue.X > 0.5f)
		...
		return;
	int32 SkillIndex = static_cast<int32>(SkillButtonDirection);
	if(UIMode == UIMODE::SWORDSTANCE)
	{
		if (CurPlayableCharacter->GetSkillActionComponent()->ExecuteSkill(SkillIndex))
		{
	...
	else if (UIMode == UIMODE::SERVANT)
	{
		if (GetFocusedServantCharacter()->GetSkillActionComponent()->ExecuteSkill(SkillIndex))
		{
			isUIMode = false;
			UIMode = UIMODE::NONE;
			UGameplayStatics::SetGlobalTimeDilation(GetWorld(), 1.0f);
			PlayerHUD->SetLinkSkillUIVisibility(ESlateVisibility::Hidden);
			ActiveUIInterface = nullptr;
			//현재 조종 중인 캐릭터가 서번트 일때만 카메라 입력 무시
			if(GetFocusedServantCharacter() == CurPlayableCharacter)
			{
				SetIgnoreLookInput(true);
			}
		}
...
void AMiyamotoIoriController::ServantUIStartedInput(const FInputActionValue& value)
{
	if (isCombat == false)
		return;
	if(isUIMode == true || UIMode != UIMODE::NONE)
		return;
	if (!SaberCharacter)
		return;
	if (!CurPlayableCharacter)
	...
	if (!HUD)
		return;
	ULinkSkillUI* LinkSkillUI = HUD->GetLinkSkillUI();
	if (!LinkSkillUI)
		return;

	isUIMode = true;
	UIMode = UIMODE::SERVANT;
	UGameplayStatics::SetGlobalTimeDilation(GetWorld(), 0.0f);
	ActiveUIInterface = LinkSkillUI;
	LinkSkillUI->SetVisibility(ESlateVisibility::SelfHitTestInvisible);
	ActiveUIInterface->OnInterfaceOpen();
}

void AMiyamotoIoriController::ServantUICompletedInput(const FInputActionValue& value)
{
	if (isCombat == false)
		return;
	if (isUIMode == false || UIMode != UIMODE::SERVANT)
		return;
	if (!SaberCharacter)
		return;
	if (!CurPlayableCharacter)
	...
	UIMode = UIMODE::NONE;
	UGameplayStatics::SetGlobalTimeDilation(GetWorld(), 1.0f);
	LinkSkillUI->SetVisibility(ESlateVisibility::Hidden);
	if(nullptr != ActiveUIInterface)
	{
		ActiveUIInterface->OnInterfaceClose();
	}
	ActiveUIInterface = nullptr;
...
{
	if (isUIMode == false || isCombat == false)
		return;
	if (UIMode != UIMODE::SERVANT)
		return;

	if (!CurPlayableCharacter || !GetFocusedServantCharacter()) return;

	AFatePlayerState* PS = GetPlayerState<AFatePlayerState>();
	if (!PS || !PS->ResonanceComponent) return;

	UResonanceComponent* ResonanceComp = PS->ResonanceComponent;
	FName ServantName = GetFocusedServantCharacter()->GetServantName();
	//이오리 일때 서번트 변경
	if (CurPlayableCharacter == MiyamotoIori)
	{
		if (ResonanceComp->IsSwapAvailable(ServantName))
		{
			SwapWithServant(GetFocusedServantCharacter());
			PlayerHUD->SetLinkSkillUIVisibility(ESlateVisibility::Hidden);
		}
		...
		{
			GEngine->AddOnScreenDebugMessage(-1, 2.0f, FColor::Yellow, TEXT("Not Enough Gauge"));
		}
	}
	//서번트를 사용하고 있는 중이면서 현재 UI포커스된 서번트와 사용 중인 서번트가 일치 될 떄
	else if(CurPlayableCharacter != MiyamotoIori && GetFocusedServantCharacter() == CurPlayableCharacter)
	{
		SwapWithServant();
		PlayerHUD->SetLinkSkillUIVisibility(ESlateVisibility::Hidden);
	}
...
void AMiyamotoIoriController::SwapWithServant(AServantBaseCharacter* TargetServant)
{
	if (!MiyamotoIori)
		return;
	...
	if (AFatePlayerState* PS = GetPlayerState<AFatePlayerState>())
	{
		...
		if (ResonanceComp)
		{
			if (CurPlayableCharacter != MiyamotoIori)
			{
				AServantBaseCharacter* CurServant = Cast<AServantBaseCharacter>(CurPlayableCharacter);
				if (nullptr == CurServant)
					return;
				ResonanceComp->SetActiveServant(CurServant->GetServantName());
			}
			else
			{
				ResonanceComp->SetActiveServant(NAME_None);
			}
		}
	}
	if (CurPlayableCharacter)
...
void AMiyamotoIoriController::HandleGaugeDepleted()
{
	bIsPendingAutoSwap = true;
}
